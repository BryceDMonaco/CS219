...
; begin initialize data
		...
		LDR r1, [r1] ;Load the count value from the address
		
		LDR r2,= 0 ;Set the tracked value to 0
		LDR r3,= 0 ;Set the greatest storage to 0
		
		LDR r4,= 0x40001010 ;Load the list start address

LOOP1	CMP r1, r2 ;Compare the count to the tracked value
		BEQ STORE ;If r1 equal r2 then the loop is done, jump to STORE
		BGT FIND32 ;If r1 is still greater then the loop must go again, go to LOOP2

FIND32	LDR r5, [r4, r2] ;Load the value at r4 with an offset of the incrementor
...
STORE	LDR r1,= 0x40001002 ;Load the storage address
		STR r3, [r1] ;Store the greatest value into the address
		BAL stop
